...
export default function DealersScreen() {
  ...
  if (loading) return (
    <SafeAreaView style={[styles.root, { justifyContent: 'center', alignItems: 'center', backgroundColor: COLORS.background }]}>
  ...
  if (error || dealers.length === 0) return (
    <SafeAreaView style={[styles.root, { justifyContent: 'center', alignItems: 'center', padding: 32, backgroundColor: COLORS.background }]}>
      <Ionicons name="location-outline" size={56} color={COLORS.textMuted} style={{ marginBottom: 16 }} />
          ...
          <Marker key={d.id} coordinate={{ latitude: d.latitude, longitude: d.longitude }} onPress={() => flyTo(d)}>
            ...
            <Callout tooltip>
              <View style={styles.callout}>
                ...
                {d.is_sponsored && (
                  <View style={styles.calloutVerifiedRow}>
                    <Ionicons name="checkmark-circle" size={12} color={COLORS.primary} />
                    <Text style={styles.calloutVerified}> Verified Dealer</Text>
                  </View>
                )}
              </View>
            </Callout>
      ...
      {dealers.length > 1 && (
        <TouchableOpacity
          ...
          onPress={() => mapRef.current?.fitToCoordinates(
            dealers.map(d => ({ latitude: d.latitude, longitude: d.longitude })),
            { edgePadding: { top: 80, bottom: 300, left: 60, right: 60 }, animated: true }
          )}
        >
      ...
      <View style={styles.panel}>
        ...
        {confidence && (
          <Text style={styles.panelSub}>
            Diagnosis confidence: {Math.round(parseFloat(confidence) * 100)}%
          </Text>
        )}
          ...
          {dealers.map((d, i) => {
            ...
            return (
              <TouchableOpacity key={d.id} style={[styles.dealerCard, isSelected && styles.dealerCardSelected]} onPress={() => flyTo(d)}>
                ...
                <View style={styles.dealerDistRow}>
...
const styles = StyleSheet.create({
  ...
  backBtn: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.primary, paddingHorizontal: 28, paddingVertical: 13, borderRadius: RADIUS.lg },
  ...
  calloutVerifiedRow: { flexDirection: 'row', alignItems: 'center', marginTop: 3 },
  calloutVerified: { fontSize: 11, color: COLORS.primary, fontWeight: '700' },

  fitBtn: { flexDirection: 'row', alignItems: 'center', position: 'absolute', top: 16, alignSelf: 'center', backgroundColor: '#fff', borderRadius: RADIUS.full, paddingHorizontal: 18, paddingVertical: 9, ...SHADOW.md },
  ...
  verifiedBadge: { flexDirection: 'row', alignItems: 'center', backgroundColor: COLORS.accentLight, borderRadius: RADIUS.full, paddingHorizontal: 8, paddingVertical: 3 },
  ...
  dealerDistRow: { flexDirection: 'row', alignItems: 'center', marginBottom: 8 },
  dealerDist: { fontSize: 12, fontWeight: '700', color: COLORS.textSecondary },
  stockRow: { flexDirection: 'row', alignItems: 'center', gap: 6, marginBottom: 10 },
  stockDot: { width: 8, height: 8, borderRadius: 4, backgroundColor: COLORS.success ?? '#16A34A' },
  stockText: { fontSize: 11, color: COLORS.textSecondary, flex: 1 },
  cardActions: { flexDirection: 'row', gap: 8 },
  callBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.background, borderRadius: RADIUS.md, paddingVertical: 9, borderWidth: 1, borderColor: COLORS.border },
  callBtnText: { fontSize: 12, fontWeight: '700', color: COLORS.primary },
  dirBtn: { flex: 1, flexDirection: 'row', alignItems: 'center', justifyContent: 'center', backgroundColor: COLORS.primary, borderRadius: RADIUS.md, paddingVertical: 9 },
